...
class MarketEnvDiscrete(MultiAgentEnv):
    ...
    def _calc_orders(self):
        ...
        """
        a = [np.arange(0.99, 3, 0.01), np.arange(0.99, 3, 0.01), np.arange(0.99, 3, 0.01)]
        combs = list(itertools.product(*a))

        results = []
        results_list = []

        for j, price_comb in enumerate(combs):
            for i, p in enumerate(price_comb):
                self.agents[i]["price"] = np.array([p])
            ...
            if sum_inverse_prices > 0:  # if at least one price is below 2
                ...
                #     orders[agent_id] += 1
                for i, b in enumerate(buying_probabilities):
                    orders[i] = b*int(max_demand_for_prices)

            revenues = [a["price"][0]*orders[a["id"]]-orders[a["id"]] for a in self.agents]
            results.append(
                {
                    "sum_revenue": sum(revenues),
                    "revenues": revenues,
                    "prices": [a["price"][0] for a in self.agents]
                }
            )
            results_list.append(
                [
                    ...
                    revenues[1],
                    revenues[2],
                    self.agents[0]["price"][0],
                    self.agents[1]["price"][0],
                    self.agents[2]["price"][0],
                ]
            )
            print(j)

        results_sorted = sorted(results, key=lambda d: d['sum_revenue'], reverse=True)
        import csv
        with open('4dData.csv', 'w', newline="") as f:
            # using csv.writer method from CSV package
            write = csv.writer(f)

            write.writerow(["sum revenue", "revenue 0", "revenue_1", "revenue_2", "price_0", "price_1", "price_2"])
